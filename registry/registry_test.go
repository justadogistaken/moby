...
func TestMirrorEndpointLookupV2(t *testing.T) {
	...
	if pushAPIEndpoints[0].URL.Host != IndexName {
		t.Fatal("push url host should be " + IndexName)
	}
	...
	if len(pullAPIEndpoints) == 0 || pullAPIEndpoints[len(pullAPIEndpoints)-1].URL.Host != IndexName {
